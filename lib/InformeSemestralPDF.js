...
export async function generarPdfSemestralBlob(datos) {
  ...
  const c = {
    ...
    cliente:        { x: 720, y: 130, size: 11, bold: false, color: negro },

    // Párrafo de intro completo (incluye la fecha integrada)
    parrafoIntro:   { x: 42, y: 152, size: 11, color: negro, lineHeight: 14, ancho: 770 },

    // Diagrama - Membrana 1 (izquierda). "Cond ___ µS/cm" arriba, "Cond ___ / Lpm" abajo
    o1CondPre1:     { x: 258, y: 218, size: 9, color: azul },   // hueco entre "Cond" y "µS/cm" (arriba izq)
    o1CondPost1:    { x: 186, y: 287, size: 9, color: verde },  // hueco entre "Cond" y "µS/cm" (abajo izq)
    o1Flujo1:       { x: 158, y: 297, size: 9, color: verde },  // antes de "Lpm" (abajo izq)

    // Diagrama - Membrana 2 (derecha)
    o1CondPre2:     { x: 426, y: 218, size: 9, color: azul },   // hueco entre "Cond" y "µS/cm" (arriba der)
    o1CondPost2:    { x: 312, y: 289, size: 9, color: verde },  // hueco entre "Cond" y "µS/cm" (abajo der)
    o1Flujo2:       { x: 284, y: 300, size: 9, color: verde },  // antes de "Lpm" (abajo der)

    // Conductividad de entrada (recuadro central): "entrada ___ µS/cm"
    cde1:           { x: 578, y: 267, size: 9, color: negro },

    // Recuadro de salida (derecha, verde)
    cds1Salida:     { x: 700, y: 244, size: 9, color: verde }, // Primer paso ___ µS/cm
    fp1:            { x: 700, y: 256, size: 9, color: verde }, // Flujo Prod ___ lpm
    fd1:            { x: 700, y: 267, size: 9, color: verde }, // Flujo Desc ___ lpm
    pd1:            { x: 700, y: 279, size: 9, color: verde }, // Pres desc ___ psi

    // Párrafo de conclusión (los valores van dentro del texto)
    parrafoConcl:   { x: 42, y: 400, size: 11, color: negro, lineHeight: 14, ancho: 770 },

    // Firma y nombre técnico
    xFirma: 380, yFirma: 490,
    xNombreCentro: 435, yNombre: 525,
  }
