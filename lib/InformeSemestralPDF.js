...
export async function generarPdfSemestralBlob(datos) {
  ...
  const c = {
    ...
    cliente:        { x: 720, y: 68, size: 11, bold: false, color: negro },
    // Párrafo de intro: "El <dia> <mes> de <anio> se procedió..."
    diaInforme:     { x: 42,  y: 155, size: 11, color: negro },
    mesInforme:     { x: 62,  y: 155, size: 11, color: negro },
    anioInforme:    { x: 120, y: 155, size: 11, color: negro },

    // Diagrama - Membrana 1 (izquierda)
    o1CondPre1:     { x: 290, y: 232, size: 9, color: azul },   // Cond pre lavado Memb 1 (arriba)
    o1CondPost1:    { x: 175, y: 305, size: 9, color: verde },  // Cond post lavado Memb 1 (abajo)
    o1Flujo1:       { x: 175, y: 318, size: 9, color: verde },  // Flujo Memb 1 (abajo)

    // Diagrama - Membrana 2 (derecha)
    o1CondPre2:     { x: 475, y: 232, size: 9, color: azul },   // Cond pre lavado Memb 2 (arriba)
    o1CondPost2:    { x: 320, y: 305, size: 9, color: verde },  // Cond post lavado Memb 2 (abajo)
    o1Flujo2:       { x: 320, y: 318, size: 9, color: verde },  // Flujo Memb 2 (abajo)

    // Conductividad de entrada (recuadro central)
    cde1:           { x: 630, y: 275, size: 9, color: negro },

    // Recuadro de salida (derecha, verde)
    cds1Salida:     { x: 745, y: 250, size: 9, color: verde }, // Primer paso <CDS1>
    fp1:            { x: 745, y: 277, size: 9, color: verde }, // Flujo Prod
    fd1:            { x: 745, y: 290, size: 9, color: verde }, // Flujo Desc
    pd1:            { x: 745, y: 303, size: 9, color: verde }, // Pres desc

    // Párrafo de conclusión (los valores van dentro del texto)
    parrafoConcl:   { x: 42, y: 420, size: 11, color: negro, lineHeight: 14, ancho: 830 },

    // Firma y nombre técnico
    xFirma: 400, yFirma: 500,
    xNombreCentro: 455, yNombre: 535,
  }
