...
export async function generarPdfSemestralBlob(datos) {
  ...
  const c = {
    ...
    cliente:        { x: 710, y: 100, size: 11, bold: false, color: negro },

    // Párrafo de intro completo (incluye la fecha integrada)
    parrafoIntro:   { x: 39, y: 145, size: 11, color: negro, lineHeight: 14, ancho: 770 },

    // Diagrama - Membrana 1 (izquierda). "Cond ___ µS/cm" arriba, "Cond ___ / Lpm" abajo
    o1CondPre1:     { x: 250, y: 215, size: 9, color: azul },   // hueco entre "Cond" y "µS/cm" (arriba izq)
    o1CondPost1:    { x: 180, y: 280, size: 9, color: verde },  // hueco entre "Cond" y "µS/cm" (abajo izq)
    o1Flujo1:       { x: 150, y: 290, size: 9, color: verde },  // antes de "Lpm" (abajo izq)

    // Diagrama - Membrana 2 (derecha)
    o1CondPre2:     { x: 420, y: 215, size: 9, color: azul },   // hueco entre "Cond" y "µS/cm" (arriba der)
    o1CondPost2:    { x: 309, y: 280, size: 9, color: verde },  // hueco entre "Cond" y "µS/cm" (abajo der)
    o1Flujo2:       { x: 280, y: 295, size: 9, color: verde },  // antes de "Lpm" (abajo der)

    // Conductividad de entrada (recuadro central): "entrada ___ µS/cm"
    cde1:           { x: 570, y: 260, size: 9, color: negro },

    // Recuadro de salida (derecha, verde)
    cds1Salida:     { x: 705, y: 240, size: 9, color: verde }, // Primer paso ___ µS/cm
    fp1:            { x: 705, y: 252, size: 9, color: verde }, // Flujo Prod ___ lpm
    fd1:            { x: 705, y: 263, size: 9, color: verde }, // Flujo Desc ___ lpm
    pd1:            { x: 705, y: 274, size: 9, color: verde }, // Pres desc ___ psi
