...
export async function generarPdfSemestral8m5s2oBlob(datos) {
  const {
    cliente,
    diaInforme, mesInforme, anioInforme,
    // Osmosis 1 (8 membranas: 1-2 en serie, 3-4-5-6 en paralelo)
    ...
    // Osmosis 2 (5 membranas, agua blanda)
  ...
  const plantillaUrl = '/plantillas/Plantilla_informe_matencion_8m_5s_2o.pdf'
  ...
  const cajaSalidaP2 = (cds, fp, fd, pd) => ([
  `Conductividad de salida ${cds} µS/cm`,
  `Flujo Prod ${fp} lpm`, 
  `Flujo Desc ${fd} lpm`,
  `Pres desc ${pd} psi`,
  ...
  const c1 = {
    ...
    m1Pre:  { x: 247, y: 207 }, m1Post: { x: 180, y: 269 },
    m2Pre:  { x: 207, y: 328 }, m2Post: { x: 195, y: 392 },
    // Membranas 3, 4, 5, 6 en paralelo (fila derecha)
    m3Pre:  { x: 341, y: 207 }, m3Post: { x: 286, y: 269 },
    m4Pre:  { x: 321, y: 326 }, m4Post: { x: 294, y: 392 },
    m5Pre:  { x: 443, y: 208 }, m5Post: { x: 392, y: 270 },
    m6Pre:  { x: 422, y: 327 }, m6Post: { x: 401, y: 392 },
    m7Pre:  { x: 535, y: 230 }, m7Post: { x: 534, y: 287 },
    m8Pre:  { x: 534, y: 344 }, m8Post: { x: 648, y: 367 },
    entrada:{ x: 407, y: 451 }, salida: { x: 519, y: 439 },
    parrafoConcl: { x: 18, y: 496, size: 11, color: negro, lineHeight: 14, ancho: 790 },
  }
  ...
  lineasEn(pag1, [`Cond ${o1m7Pre} µS/cm`], c1.m7Pre.x, c1.m7Pre.y, azul)
  lineasEn(pag1, [`Cond ${o1m7Post} µS/cm`, `${o1m7Flujo} Lpm`], c1.m7Post.x, c1.m7Post.y, verde)
  lineasEn(pag1, [`Cond ${o1m8Pre} µS/cm`], c1.m8Pre.x, c1.m8Pre.y, azul)
  lineasEn(pag1, [`Cond ${o1m8Post} µS/cm`, `${o1m8Flujo} Lpm`], c1.m8Post.x, c1.m8Post.y, verde)
  ...
  const c2 = {
    cliente:  { x: 680, y: 73 },
    m1Pre:  { x: 197, y: 175 }, m1Post: { x: 191, y: 243 },
    m2Pre:  { x: 312, y: 175 }, m2Post: { x: 317, y: 242 },
    m3Pre:  { x: 427, y: 175 }, m3Post: { x: 434, y: 242 },
    m4Pre:  { x: 544, y: 177 }, m4Post: { x: 550, y: 242 },
    m5Pre:  { x: 657, y: 175 }, m5Post: { x: 660, y: 240 },
    entrada:{ x: 374, y: 306 }, salida: { x: 494, y: 291 },
    parrafoConcl: { x: 18, y: 372, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    xFirma: 370, yFirma: 500, xNombre: 415, yNombre: 506,
  }
  ...
  lineasEn(pag2, cajaSalidaP2(o2cds, o2fp, o2fd, o2pd), c2.salida.x, c2.salida.y, verde)
  const yFinConcl2 = parrafoEn(pag2, `Después del procedimiento de lavado químico (ac. cítrico como desincrustante y ac. peracético como desinfectante), el sistema quedó con una mejor performance, conductividad de salida de ${o2cds} µS/cm, con lo cual estamos en una RR cercana al ${rr2Texto}% ${valFinal2}. También se verificaron los flujos normales (3-5 lts. por membrana). La presión de rechazo quedó en ${o2pd} psi y ${o2fp} lpm de flujo producto.`, c2.parrafoConcl)
