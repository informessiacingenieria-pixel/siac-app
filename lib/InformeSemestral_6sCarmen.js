...
export async function generarPdfSemestral6sCarmenBlob(datos) {
  ...
  const c1 = {
    ...
    entrada:{ x: 434, y: 478 }, salida: { x: 562, y: 466 },
  }
  ...
  const c2 = {
    ...
    yNombre: 405,  // Coordenada Y del nombre
  }
  ...
  const yFinConcl1 = parrafo(page2, `Después del procedimiento del lavado químico, con ácido cítrico al 3% por 45 minutos como desincrustante y luego expuesto a la acción de ácido Peracético al 1% por 30 minutos como desinfectante, el sistema quedo con una salida de ${cds} µS/cm, con una RR de ${rrTexto}% ${valFinal}. También se verificaron los flujos por membrana (normal 4 - 5 lts.) y las presiones, quedando en ${pd} psi de presión de rechazo y ${fp} lpm de flujo de permeado.`, c2.parrafoConcl)
  const obs1 = (observacion || '').trim()
  if (obs1) parrafo(page2, `Observación: ${obs1}`, { ...c2.parrafoConcl, y: yFinConcl1 + 6 })
