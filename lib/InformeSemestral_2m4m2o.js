...
// Colores exactos de la plantilla
const VERDE = [0.0, 0.6902, 0.3137]
const AZUL  = [0.0941, 0.4902, 0.7765]

export async function generarPdfSemestral2m4m2oBlob(datos) {
  const {
    cliente,
  ...
  const plantillaUrl = '/plantillas/Plantilla_informe_matencion_2m_4m_2o.pdf'
  ...
  // ===================== PÁGINA 1 - OSMOSIS 1 (2 membranas) =====================
  const c1 = {
    cliente:      { x: 680, y: 73 },
    parrafoIntro: { x: 18, y: 132, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    m1Pre:  { x: 225, y: 220 }, m1Post: { x: 217, y: 300 },
    m2Pre:  { x: 353, y: 220 }, m2Post: { x: 355, y: 300 },
    entrada:{ x: 480, y: 250 }, salida: { x: 595, y: 232 },
    parrafoConcl: { x: 30, y: 415, size: 11, color: negro, lineHeight: 14, ancho: 790 },
  }
  ...
  const yFinConcl1 = parrafoEn(pag1, `Después del procedimiento de lavado químico (ac. cítrico como desincrustante y ac. peracético como desinfectante), la conductividad de salida quedó en ${o1cds} µS/cm, con lo cual estamos en una RR cercana al ${rr1Texto}% ${valFinal1}. También se verificaron los flujos normales (normal 3-5 lts. por membrana). La presión de rechazo quedó en ${o1pd} psi y ${o1fp} lpm de flujo de permeado. Se vuelve a configuración para trabajar como ósmosis en línea.`, c1.parrafoConcl)
  ...
  // Las 4 membranas se distribuyen horizontalmente. Coordenadas de partida (a afinar).
  const c2 = {
    m1Pre: { x: 219, y: 175 }, m1Post: { x: 218, y: 245 },
    m2Pre: { x: 352, y: 175 }, m2Post: { x: 345, y: 245 },
    m3Pre: { x: 477, y: 175 }, m3Post: { x: 477, y: 245 },
    m4Pre: { x: 609, y: 175 }, m4Post: { x: 605, y: 245 },
    entrada:{ x: 405, y: 310 }, salida: { x: 525, y: 298 },
    parrafoConcl: { x: 18, y: 375, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    xFirma: 375, yFirma: 500, xNombre: 415, yNombre: 507,
  }
  lineasEn(pag2, [cliente], c1.cliente.x, c1.cliente.y, negro, 11)
