...
// Rango de litros por centro (fijo, se usa en ambas osmosis)
const LITROS_POR_CENTRO = {
  'Davila UCI': '(normal 3-5 lts. por membrana)',
  'Municipalidad Puerto Montt': '(normal 10-25 lts. por membrana)',
}
const LITROS_DEFECTO = '(normal 3-5 lts. por membrana)'
...
export async function generarPdfSemestral5s2s2oBlob(datos) {
  const {
    cliente,
    ...
    // Osmosis 2 (2 membranas, agua blanda)
  ...
  const plantillaUrl = '/plantillas/Plantilla_informe_mantencion_5s_2s_2o.pdf'
  ...
  function parrafoEn(page, texto, conf) {
    ...
    return conf.y + lineas.length * conf.lineHeight
  }

  const mesNombre = MESES_NOMBRE[parseInt(mesInforme) - 1] || ''
  const litros = LITROS_POR_CENTRO[cliente] || LITROS_DEFECTO
  ...
  // ===================== PÁGINA 1 - OSMOSIS 1 (5 membranas) =====================
  const c1 = {
    cliente:      { x: 655, y: 73 },
    parrafoIntro: { x: 18, y: 137, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    m1Pre:  { x: 168, y: 213 }, m1Post: { x: 165, y: 280 },
    m2Pre:  { x: 300, y: 212 }, m2Post: { x: 292, y: 280 },
    m3Pre:  { x: 157, y: 320 }, m3Post: { x: 153, y: 380 },
    m4Pre:  { x: 298, y: 320 }, m4Post: { x: 297, y: 379 },
    m5Pre:  { x: 480, y: 251 }, m5Post: { x: 478, y: 319 },
    entrada:{ x: 584, y: 297 }, salida: { x: 705, y: 280 },
    parrafoConcl: { x: 18, y: 475, size: 11, color: negro, lineHeight: 14, ancho: 790 },
  }
  ...
  const yFinConcl1 = parrafoEn(pag1, `Después del procedimiento de lavado químico (ac. cítrico como desincrustante y ac. peracético como desinfectante), el sistema quedó con una mejor performance, conductividad de salida de ${o1cds} µS/cm, con lo cual estamos en una RR cercana al ${rr1Texto}% ${valFinal1}. También se verificaron los flujos normales ${litros}. La presión de rechazo quedó en ${o1pd} psi y ${o1fp} lpm de flujo producto.`, c1.parrafoConcl)
  const obs1 = (o1observacion || '').trim()
  if (obs1) parrafoEn(pag1, `Observación: ${obs1}`, { ...c1.parrafoConcl, y: yFinConcl1 + 6 })

  // ===================== PÁGINA 2 - OSMOSIS 2 (2 membranas, agua blanda) =====================
  const c2 = {
    cliente:  { x: 655, y: 73},
    m1Pre:  { x: 285, y: 150.5 }, m1Post: { x: 284, y: 216 },
    m2Pre:  { x: 519, y: 150.5 }, m2Post: { x: 518, y: 217 },
    entrada:{ x: 492, y: 283 }, salida: { x: 615, y: 265 },
    parrafoConcl: { x: 18, y: 360, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    xFirma: 365, yFirma: 505.5, xNombre: 415, yNombre: 507,
  }
  ...
  const yFinConcl2 = parrafoEn(pag2, `Después del procedimiento de lavado químico (ac. cítrico como desincrustante y ac. peracético como desinfectante), la conductividad de salida quedó en ${o2cds} µS/cm, con lo cual estamos en una RR cercana al ${rr2Texto}% ${valFinal2}. También se verificaron los flujos normales ${litros}. La presión de rechazo quedó en ${o2pd} psi y ${o2fp} lpm de flujo de permeado. Se vuelve a configuración para trabajar como ósmosis en línea.`, c2.parrafoConcl)
  const obs2 = (o2observacion || '').trim()
  if (obs2) parrafoEn(pag2, `Observación: ${obs2}`, { ...c2.parrafoConcl, y: yFinConcl2 + 6 })
