...
export async function generarPdfSemestral3s1oBlob(datos) {
  ...
  const c = {
    ...
    // 3 membranas en fila
    m1Pre:  { x: 212, y: 220 }, m1Post: { x: 210, y: 290 },
    m2Pre:  { x: 322, y: 220 }, m2Post: { x: 326, y: 290 },
    m3Pre:  { x: 437, y: 220 }, m3Post: { x: 442, y: 290 },

    entrada:{ x: 569, y: 246 }, salida: { x: 689, y: 232 },

    parrafoConcl: { x: 18, y: 390, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    parrafoObs: { x: 18, y: 460, size: 11, color: negro, lineHeight: 14, ancho: 790 },
  ...
  const yFinConcl1 = parrafo(`Después del procedimiento de lavado químico (ac. cítrico como desincrustante y ac. peracético como desinfectante), la conductividad de salida quedó en ${cds} µS/cm, con lo cual estamos en una RR cercana al ${rrTexto}% ${valFinal}. También se verificaron los flujos normales (normal 3-5 lts. por membrana). La presión de rechazo quedó en ${pd} psi y ${fp} lpm de flujo de permeado. Se vuelve a configuración para trabajar como ósmosis en línea.`, c.parrafoConcl)
  const obs1 = (observacion || '').trim()
  if (obs1) parrafo(`Observación: ${obs1}`, { ...c.parrafoConcl, y: yFinConcl1 + 6 })
  if (observaciones && observaciones.trim()) {           // ← AGREGAR ESTE BLOQUE
    parrafo(`Observaciones: ${observaciones.trim()}`, c.parrafoObs)
  }
