...
export async function generarPdfSemestral3sOtro1oBlob(datos) {
  ...
  const c1 = {
    ...
    m3Pre:  { x: 560, y: 250.5 }, m3Post: { x: 565, y: 320 },
    entrada:  { x: 320, y: 390 },
  ...
  const c2 = {
    cliente:      { x: 680, y: 80 },
    parrafoConcl: { x: 20, y: 130, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    nota:         { x: 20, y: 195, size: 11, color: negro, lineHeight: 14, ancho: 790 },
    xFirma: 360, yFirma: 420, xNombre: 420, yNombre: 419.5,
  }

  lineasEn(pag2, [cliente], c2.cliente.x, c2.cliente.y, negro, 11)
  const yFinConcl1 = parrafoEn(pag2, `Después del procedimiento de lavado químico (ac. cítrico como desincrustante y ac. peracético como desinfectante), la conductividad de salida quedo en ${cds1} µS/cm, con lo cual estamos en una RR cercana al ${rr1Texto}% ${valFinal}. También se verificaron los flujos normales (normal 3-5 lts. por membrana). La presión de rechazo quedo en ${pd1} psi y ${fp1} lpm de flujo de permeado. Se vuelve a configuración para trabajar como osmosis en línea.`, c2.parrafoConcl)
  const obs1 = (observacion || '').trim()
  if (obs1) parrafoEn(pag2, `Observación: ${obs1}`, { ...c2.parrafoConcl, y: yFinConcl1 + 6 })
