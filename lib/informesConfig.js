...
// Formatea día/mes/año a texto tipo "25 de Junio 2026"
...
// Formatea día/mes/año a DD/MM/YYYY
...
// Calcula hora de término = hora inicio + tiempo estadía + tiempo enjuague (minutos)
...
// Calcula concentración: % y PPM = (lt quimico / (lt agua + lt quimico)) * 100
export function calcularConcentracion(ltAgua, ltQuimico) {
  const agua = parseFloat(String(ltAgua).replace(',', '.')) || 0
  const quimico = parseFloat(String(ltQuimico).replace(',', '.')) || 0
  const total = agua + quimico
  if (total === 0) return '0% o 0 PPM'
  const porcentaje = (quimico / total) * 100
  const ppm = porcentaje * 10000
  return `${porcentaje.toFixed(2)}% o ${Math.round(ppm)} PPM`
}
